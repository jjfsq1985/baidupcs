...
void std_string_verifycode(char *str, int size)
{
    char cBuffer[64];
    gets(cBuffer);
    //中文转换成宽字符
    int nWSize = MultiByteToWideChar(CP_ACP, 0, cBuffer, -1, NULL, 0);
    wchar_t *data = (wchar_t*)pcs_malloc(nWSize*sizeof(wchar_t));
    MultiByteToWideChar(CP_ACP, 0, cBuffer, -1, data, nWSize);
    //宽字符再转成UTF-8格式的中文
    int nUtf8len = WideCharToMultiByte(CP_UTF8, 0, data, -1, NULL, 0, NULL, NULL);
    if (nUtf8len >= 64)
    {
        pcs_free(data);
        return;
    }
    char* cUtf8Buffer = (char*)pcs_malloc(nUtf8len*sizeof(char));
    WideCharToMultiByte(CP_UTF8, 0, data, -1, cUtf8Buffer, nUtf8len, NULL, NULL);
    pcs_free(data);
    strcpy(cBuffer, cUtf8Buffer);
    pcs_free(cUtf8Buffer);
    //cBuffer中就是验证码
    int nLen = strlen(cBuffer);
    if (nLen < size)
    {
        strcpy(str, cBuffer);
        str[nLen] = '\0';
